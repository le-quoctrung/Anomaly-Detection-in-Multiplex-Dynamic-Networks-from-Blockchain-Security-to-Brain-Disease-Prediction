...
class ANOMULY(nn.Module):
    def __init__(self, n_views, num_features, hidden_dims, gru_hidden_dims, attns_in_channels, attns_out_channels, n_layers=2) -> None:
        ...
        self.graph_gcn_layers = nn.ModuleList([nn.ModuleList([]) for _ in range(self.n_views)])
        self.graph_gru_layers = nn.ModuleList([nn.ModuleList([])  for _ in range(self.n_views)])
        self.batch_norm = nn.ModuleList([nn.ModuleList([])  for _ in range(self.n_views)])
        self.attns = []
        ...
        for layer in range(self.n_layers):     
            for view in range(self.n_views):
                    if layer == 0:
                        ...
                        self.graph_gcn_layers[view].append(GCNConv(gru_hidden_dims[layer - 1], hidden_dims[layer]))
    ...
    def forward(self, data, hiddens=None):
        ...
        if hiddens is None:
            hiddens = [None for _ in range(self.n_views)]
            for view in range(self.n_views):
                hiddens[view] = self.init_hidden(self.gru_hidden_dims[view])
